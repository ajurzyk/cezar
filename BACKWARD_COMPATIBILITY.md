...
- Workspace: `GET/POST /api/projects`, `DELETE /api/projects/:projectId`, `POST /api/projects/checkout`, `GET /api/fs/browse` — the registered-project list and same-origin project-management/folder-browsing surface for the cockpit sidebar. The GET shape is `{projects: [{id, name, root, branch?, status, source, lastOpenedAt}], bootProject, projectsDir}` (`status`: `ok`/`missing`/`not-git`; `branch` only when cheaply readable). Same-origin only — no CORS, which is exactly what licenses the absolute `root`s that health must never carry. The list never 404s: an empty or unreadable registry answers `projects: []`.
