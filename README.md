...
remote whose host isn't `github.com` — but that override is only useful when
the remote host actually is github.com. On any other host it is not a working
...
someone else's repository, or none at all. Parametrizing the driver's own host
is a later stage; `apiUrl`/`webUrl` are ignored for `kind: "github"` regardless.
...
parsed; a missing field means it degraded silently — recheck `kind`, `apiUrl`
and `webUrl` for typos.
