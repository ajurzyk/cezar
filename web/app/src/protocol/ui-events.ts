/**
 * Normalized agent-event protocol v2 — the cockpit bundle's mirror of
 * `src/core/ui-events.ts` (Step 2.4 of the R2 plan).
 *
 * Why a mirror and not an import: same reason as `api/types.ts` — the server
 * side lives under NodeNext and the bundle under Vite's bundler resolution,
 * and the types are the contract, not the module graph. The mirror is
 * *checked*, not trusted: `src/server/api-types.test.ts` asserts type
 * exactness between every declaration here and the server's own, so a drift
 * fails `npm run typecheck` (the gate) instead of the UI at runtime.
 *
