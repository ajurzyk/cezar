/**
 * Tool display model — the cockpit bundle's mirror of
 * `src/core/tool-display.ts`, computed ONCE in the protocol layer so the
 * thread, activity groups and notifications all say the same thing.
 *
 * Mirrored rather than shared for the same reason as `ui-events.ts`: one
 * implementation would have to be reachable from both the NodeNext server
 * program and the Vite bundle, coupling the bundle's inputs to `src/` —
 * exactly the split `api/types.ts` documents. The mirror is *checked*, not
 * trusted, twice over: `src/server/api-types.test.ts` guards the types, and
 ...
 *
 * The `.js` specifier below is deliberate: Vite maps it to `ui-events.ts`
 * for the bundle, and it is what lets the NodeNext-side drift tests import
 * this module at all.
 *
