...
describe("GET /api/fs/browse (step 4.1)", () => {
  ...
  beforeEach(() => {
    ...
    outside = realpathSync(
      mkdtempSync(join(tmpdir(), "cez-fs-browse-outside-")),
    );
  ...
  const error = async (res: Response) =>
  ...
  it("showHidden=1 opts dotfolders back in", async () => {
    expect(names(await body(await browse("?showHidden=1")))).toContain(
      ".hidden",
    );
  });
  ...
  it("flags git repos with isRepo and still lists plain folders", async () => {
    const payload = await body(
      await browse(`?path=${encodeURIComponent(join(home, "projects"))}`),
    );
  ...
  it("a relative path resolves against the root", async () => {
    expect((await body(await browse("?path=projects"))).path).toBe(
      join(home, "projects"),
    );
  });
  ...
  it("rejects `..` traversal out of the root (absolute and relative spellings)", async () => {
    ...
    const doubled = await browse(
      `?path=${encodeURIComponent(encodeURIComponent("../../"))}`,
    );
  ...
  it("rejects an absolute path outside the root — without echoing it back", async () => {
    const res = await browse(
      `?path=${encodeURIComponent(join(outside, "secrets"))}`,
    );
  ...
  it("rejects a SYMLINK that escapes the root, even though it spells as inside", async () => {
    ...
    for (const path of [
      join(home, "link-outside"),
      join(home, "link-outside/secrets"),
    ]) {
      const res = await browse(`?path=${encodeURIComponent(path)}`);
      expect(res.status, path).toBe(400);
      const message = await error(res);
  ...
  it("never LISTS an escaping symlink either (no click needed to learn it exists)", async () => {
    ...
    expect(
      (
        await body(
          await browse(
            `?path=${encodeURIComponent(join(home, "link-inside"))}`,
          ),
        )
      ).path,
    )
  ...
  it("404s for a missing path, a file, and a NUL byte — never a filesystem detail", async () => {
    expect(
      (await browse(`?path=${encodeURIComponent(join(home, "nope"))}`)).status,
    ).toBe(404);
    // A file answers exactly like an absent path: confirming "this is a file"
    // would leak a name the picker never showed.
    const fileRes = await browse(
      `?path=${encodeURIComponent(join(home, "notes.txt"))}`,
    );
  ...
  describe("hosted mode (CEZ_REMOTE=1) narrows the root to projectsDir", () => {
    ...
    it("rejects the home directory a local cockpit would happily list", async () => {
      ...
      expect((await browse(`?path=${encodeURIComponent(home)}`)).status).toBe(
        200,
      );
    });
    ...
    it("404s when projectsDir does not exist yet", async () => {
      writeFileSync(
        workspaceConfigPath(),
        JSON.stringify({ projectsDir: "~/not-created" }),
        "utf8",
      );
      const res = await browse();
  ...
  it("is workspace-level: never mirrored under /api/p/", async () => {
    expect((await apiRequest(app, "/api/p/default/fs/browse")).status).toBe(
      404,
    );
  });
});
