...
describe("project-route alias parity (unprefixed vs /api/p/<boot> vs /api/p/default)", () => {
  ...
  const expectParity = async (
    ...
    const normalize = path.startsWith("/github")
      ? stripTimestamps
    ...
    for (const other of [scoped, aliased]) {
      expect(other.status, `${other.url} vs ${unprefixed.url}`).toBe(
        unprefixed.status,
      );
  ...
  it("answers every GET route byte-identically across the three spellings", async () => {
    ...
    const gets = manifest.filter(
      (r: ProjectRouteInfo) =>
        r.method === "GET" && !r.path.endsWith("/events"),
    );
  ...
  it("keeps mutating-route rejections byte-identical (targeted non-GET cases)", async () => {
    ...
    await expectParity(
      "/agent-config/no-such-file",
      json("PUT", { content: "{}", version: null }),
    );
  ...
  it("SSE: all three spellings of /events connect and open identically", async () => {
    ...
    const [unprefixed, scoped, aliased] = await Promise.all([
      sseHead(a),
      sseHead(b),
      sseHead(c),
    ]);
  ...
  it("unknown and malformed project ids answer 404 {error}", async () => {
    for (const url of [
      "/api/p/no-such-project/runs",
      "/api/p/no-such-project/events",
    ]) {
      const res = await apiRequest(app, url);
  ...
  it("a non-boot project builds lazily and answers with its own state", async () => {
    ...
    const otherKey = (
      (await (
        await apiRequest(app, `/api/p/${other.id}/launch-key`)
      ).json()) as {
        key: string;
      }
    ).key;
  ...
  it("agent config resolves repo-local files from the selected project only", async () => {
    ...
    const boot = await apiRequest(
      app,
      "/api/agent-config/claude.project.settings",
    );
    ...
    expect(
      (await (
        await apiRequest(app, "/api/agent-config/claude.project.settings")
