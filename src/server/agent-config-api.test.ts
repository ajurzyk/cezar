...
describe("the agent-config API", () => {
  ...
  it("PUT rejects invalid JSON with 400", async () => {
    expect(
      (await put("claude.project.settings", { content: "{bad", version: null }))
        .status,
    ).toBe(400);
  });
  ...
  it("PUT :id → 404 for an unknown id", async () => {
    expect((await put("nope", { content: "x", version: null })).status).toBe(
      404,
    );
  });
  ...
  it("hosted mode: repo-file reads work, home-dir file reads are withheld, userMcp is null", async () => {
    ...
    expect(
      (await apiRequest(app, "/api/agent-config/claude.user.settings")).status,
    ).toBe(409);
    expect(
      (await apiRequest(app, "/api/agent-config/codex.user.config")).status,
    ).toBe(409);
  });
});
