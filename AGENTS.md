...
| HTTP server & API routes | `src/server/server.ts` | Hono app, binds to `127.0.0.1` only. A global `/api/*` request-origin guard (#426) runs before every route except `/api/health`: it rejects non-loopback `Host` headers (DNS-rebinding) in local mode and cross-origin mutating requests (CSRF) in both modes — zero-config, no token, so the same-origin cockpit and the Vite dev proxy pass untouched. Every mutating route validates its body with a zod `safeParse` and returns `{ error }` with 400/404/409 — follow that pattern exactly. CORS is deliberately enabled for `/api/health` only (bookmarklets); never widen it. SSE endpoints replay from NDJSON then stream live, deduped by `seq`. |
