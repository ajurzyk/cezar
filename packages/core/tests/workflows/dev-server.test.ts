...
function makeFakeGitHub(): WorkflowGitHub {
  ...
  return {
    ...
    async updateComment() {},
    async getIssueWithComments(n) { return { issue: { number: n, title: `Bug #${n}`, body: 'breaks' }, comments: [] }; },
    async setLabels() {}, async addLabel() {}, async closeIssue() {},
    async pushBranch() {},
    async createPullRequest() { return { url: 'https://example.test/pr/9', number: 9 }; },
  };
}
...
class CapturingRunner implements AgentRunner {
  ...
  async run<T>(spec: AgentRunSpec<T>): Promise<AgentRunResult<T>> {
    ...
    return { text: JSON.stringify(parsed), parsed, toolCalls: [], tokensUsed: 5, budgetExceeded: false };
  }
...
function makeFakeGit() {
  let n = 0;
  return { async commitAll(): Promise<string | null> { n++; return `sha${n}00000`; }, async getDiffAgainstBase(): Promise<string> { return 'diff'; } };
}

const ok = (cmd: string): ShellResult => ({ command: cmd, ok: true, exitCode: 0, stdout: '', stderr: '', durationMs: 1 });

/** Fake env whose dev server boots to a fixed URL. */
function makeFakeEnv(opts: { devUrl?: string; ready?: boolean; throwOnStart?: boolean } = {}): RunEnv & { started: number } {
  const state = { started: 0 };
  return {
    started: 0,
    kind: 'native',
    async install() { return ok('install'); },
    async build() { return null; },
    async test() { return null; },
    async run(c) { return ok(c); },
...
describe('autofix dev-server step', () => {
  it('boots the dev server and injects the URL + curl into agent steps', async () => {
    ...
    const result = await runWorkflow(autofixWorkflow, {
      ...
      github: makeFakeGitHub(), issueNumber: 20, apply: true, worktreePath: '/tmp/wt',
    ...
    const fixSpec = runner.specs.find((s) => s.userPrompt.includes('guard') || s.systemPrompt.includes('FIXER'))
      ?? runner.specs[2];
  ...
  it('continues (no URL) when the dev server is not configured', async () => {
    ...
    const result = await runWorkflow(autofixWorkflow, {
      store: await makeStore(), config: makeConfig({}),
      github: makeFakeGitHub(), issueNumber: 21, apply: true, worktreePath: '/tmp/wt',
      runnerFactory: () => runner, gitOps: makeFakeGit(), runEnv: env,
      loopMaxIterations: { 'fix-review': 2 },
    });
    expect(result.status).toBe('succeeded');
  ...
  it('does not fail the run when boot throws (ungated dev-server step)', async () => {
    ...
    const result = await runWorkflow(autofixWorkflow, {
      store: await makeStore(), config: makeConfig({ devServer: { enabled: true, command: 'yarn dev', port: 3000 } }),
      github: makeFakeGitHub(), issueNumber: 22, apply: true, worktreePath: '/tmp/wt',
      runnerFactory: () => runner, gitOps: makeFakeGit(), runEnv: env,
      loopMaxIterations: { 'fix-review': 2 },
    });
