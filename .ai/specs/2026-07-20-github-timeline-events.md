...
for (let page = 1; page <= TIMELINE_MAX_PAGES; page++) {
  ...
  const out = await gh(repoRoot, [...], remaining);
  …
}
```

A budget exhausted mid-loop is treated exactly like the page cap: keep what arrived, set `truncated`. With that, the whole timeline fetch is bounded by **one 15 s window**, the per-call worst case matches the pre-change shape, and the paging decision costs nothing in latency ceiling. Unit test: a loop whose pages each consume most of the budget stops early and sets `truncated` rather than running ten full windows.
...
The fetch therefore carries a **comments top-up safeguard**: when the fetch stopped short (page cap or time budget) *and* fewer than `THREAD_ENTRY_CAP` `commented` rows were seen, re-fetch the comment stream with the legacy `/issues/{n}/comments --paginate` call and use it in place of the timeline's `commented` rows. `comments[]` then stays byte-identical to today's output in every case, not just the common one.
...
**A failed top-up degrades to the timeline's own comments, never to `[]`.** The top-up runs inside the same inner `try` as the timeline fetch (see §API Contracts), but its failure must not reach even that handler's fallback — the fallback *is* the same call. On a top-up throw, keep the `commented` rows the timeline already returned and leave `truncated: true`. That is at worst the short list the safeguard existed to avoid, and never the empty thread the outer handler would produce; a safeguard that can fail *worse* than its own absence would be a bug. Unit tests: a page-capped, comment-poor prefix returns the full pre-change `comments[]`; a top-up that throws still returns the timeline's `commented` rows with `available: true`.
...
| Timeline 404 / 5xx / non-`403` non-zero exit | **Yes** | Endpoint-specific (GHES variance, token scope, endpoint unavailability). `/issues/{n}/comments` is a *different* endpoint and typically still succeeds. |
...
| Timeline budget exhausted (15 s total) | **Yes**, once | Accepted: the alternative is silently returning an empty thread, and a slow network is exactly when the fallback earns its cost. See the worst-case note below. |
