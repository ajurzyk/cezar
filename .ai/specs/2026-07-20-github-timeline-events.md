...
The `Accept` header is carried over from #525 step 2 and is pinned deliberately rather than left to `gh`'s default. Current `gh` already sends a compatible media type, so this changes nothing today. It is pinned because the timeline is the one endpoint in this spec with a history of media-type-gated behavior — it spent years behind the `mockingbird-preview` media type — which makes it the endpoint where an implicit client default is least worth relying on. One flag, no behavior change, one less silent dependency. The existing calls at `:455`/`:458` omit it and are deliberately left alone; retrofitting them is unrelated churn.
...
4. `src/server/forge/github.ts`: in `fetchGithubComments` (`:443-474`), replace the `:455` call with a **manual 10-page loop** over `/issues/{n}/timeline?per_page=100&page={n}`, passing `-H "Accept: application/vnd.github+json"` (per #525 step 2) — **not** `--paginate`, which has no page cap (see §Architecture "Pagination") — stopping early on a short page; split rows — `event === 'commented'` through the existing `normalizeComments` (`:371`), the rest through `normalizeEvents`. Keep the `:458` reviews call unchanged. Wrap in try/catch falling back to the old comments call on **every endpoint-level failure including 404**; skip the fallback **only** on `ENOENT` (no `gh` binary), per the degradation table in §API Contracts. Set `truncated` when the page cap is hit. Unit tests: split correctness; **`comments[]` byte-identical to pre-change output for the same input** (the §2 guarantee); **a timeline 404 still returns populated `comments[]`**; `ENOENT` spawns no second call; page-cap truncation.
