...
- **BACKWARD_COMPATIBILITY.md**: all existing HTTP routes, NDJSON v1 event types, `runs.json`, workflow YAML, CLI flags and bookmarklet URLs are preserved; every schema change is additive; `web/` raw files stop shipping in the npm tarball (replaced by `web/dist`) — noted as a packaging change with a minor version bump.
